...
{
    std::unique_ptr<NET_SCAN_TRACK_COLLECT_SAMELENGTH> samelengt_tracks(
        new NET_SCAN_TRACK_COLLECT_SAMELENGTH( aTrackFrom, this, aUndoRedoList ) );
    ...
    if( samelengt_tracks )
    {
        ...
        if( tracks_arced.size() > 5 ) //Bunch of segments not only one or two.
        {
            ...
            std::map<TRACK*, bool> tracks_connected2arced = FindSegmentsBothSidesOfArced( &tracks_arced,
                                                                                          tracks_other );
            ...
            if( tSamelength )
            {
                ...
                for( auto tArced : tracks_arced )
                {
                    if( tArced )
                    {
                        ...
                        if( ( tSamelength_start == tArced_start ) ||
                            ( tSamelength_start == tArced_end ) ||
                            ( tSamelength_end == tArced_start ) ||
                            ( tSamelength_end == tArced_end ) )
                        {
        ...
        if( segArced )
        {
            ...
            for( auto segOther : *aTracksOthers )
            {
                if( segOther )
                {
                    ...
                    if( ( segArced_start == segOther_start ) ||
                        ( segArced_start == segOther_end ) ||
                        ( segArced_end == segOther_start ) ||
                        ( segArced_end == segOther_end ) )
                    {
                        bool startpoint = ( ( segOther_start == segArced_start ) ||
                                            ( segOther_start == segArced_end ) );
                        tracks_connected2arced.insert( std::pair<TRACK*,
                                                       bool>( segOther,startpoint ) );
                    }
                }
            }
        }
                                       ...
                                       )
{
    ...
    if( Rad2MilsInt( first_track_angle ) == Rad2MilsInt( second_track_angle ) )
    {
        ...
        double half_dist_btw_tracks = GetLineLength( first_track_connected_pos,
                                                     second_track_connected_pos ) / 2.0;

        wxPoint first_track_new_pos = GetPoint( first_track_connected_pos,
                                                first_track_angle + M_PI,
                                                half_dist_btw_tracks );
        ...
        wxPoint second_track_new_pos = GetPoint( second_track_connected_pos,
                                                 second_track_angle + M_PI,
                                                 half_dist_btw_tracks );
    ...
    else 
    {
        double angle_btw_tracks = AngleBtwTracks( first_track,
                                                  ...
                                                  second_track,
                                                  second_track_connected_pos );

        double virtual_track_length = GetLineLength( first_track_opposite_pos,
                                                     second_track_opposite_pos );

        RemoveArcedSegments( aTracksArced, aUndoRedoList );

        angle_btw_tracks = InnerAngle( angle_btw_tracks );

        double virtual_trac_angle = AngleRad( first_track_opposite_pos,
                                              second_track_opposite_pos );

        double first_trac_opposite_angle = TrackSegAngle( first_track, first_track_opposite_pos );
        double angle_first_btw_virtual = NormAngle( first_trac_opposite_angle - virtual_trac_angle,
                                                    0.0,
                                                    M_PIx2,
                                                    M_PIx2 );
        ...
        double second_track_length = GetLineLength( second_track->GetStart(),
                                                    second_track->GetEnd() );

        wxPoint common_point = GetPoint( second_track_opposite_pos,
                                         second_track_angle + M_PI,
                                         second_track_full_length );
        ...
        //Jos meni pieleen ; )
        double new_angle_btw_tracks = InnerAngle( AngleBtwTracks( first_track,
                                                                  common_point,
                                                                  second_track,
                                                                  common_point ) );
        ...
        {
            common_point = GetPoint( second_track_opposite_pos,
                                     second_track_angle,
                                     second_track_full_length );
        ...
        Add( first_track,
             common_point,
             second_track_full_length - second_track_length,
             aUndoRedoList );
