...
// PLOT
//-----------------------------------------------------------------------------------------------------/
void TEARDROPS::Plot( const TRACKNODEITEM* aTrackNodeItem,
                      PLOTTER* aPlotter,
            ...
            switch( tear->GetShape() )
            {
                ...
                case TEARDROP::TEARDROP_T:
                    for( unsigned int n = 0; n < tear->GetPolyPointsNum() - 1; n++ )
                    {
                        ...
                        aPlotter->ThickSegment( tear->GetPolyPoint( n ),
                                                tear->GetPolyPoint( n+1 ),
                                                tear->GetPolySegmentWidth(),
                                                *aPlotMode,
                                                aData );
                    }
                ...
                case TEARDROP::SUBLAND_T:
                    aPlotter->FlashPadCircle( tear->GetPosition(),
                                              tear->GetWidth(),
                                              *aPlotMode,
                                              aData );
            }
