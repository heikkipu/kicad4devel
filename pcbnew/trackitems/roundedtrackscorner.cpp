...
ROUNDEDTRACKSCORNER::ROUNDEDTRACKSCORNER( const BOARD_ITEM* aParent,
    ...
    TRACKNODEITEM( const_cast<BOARD_ITEM*>( aParent ), PCB_ROUNDEDTRACKSCORNER_T )
{
    m_set_ok = true;
...
void ROUNDEDTRACKSCORNER::SetTrackSecondEndpoint( void )
{
    if( m_trackseg_second )
    {
        if( ( m_connected_pos == m_trackseg_second->GetStart() ) ||
            ( m_connected_pos == m_trackseg_second->GetEnd() ) )
        {
...
bool ROUNDEDTRACKSCORNER::Update( void )
{
    ...
    if( ( m_angle_btw_tracks == M_PI ) || 
        ( Rad2MilsInt( m_angle_inner_btw_tracks ) < RAD_90_MILS_INT ) || 
        !AreTracksConnected()
  )
    {
        SetNotOKValues();
...
void ROUNDEDTRACKSCORNER::AddTo3DContainer( CBVHCONTAINER2D* aContainer, const double aBiuTo3Dunits )
{
    if( IsSetOK() && m_on )
    {
        for( unsigned int n = 0; n < m_seg_points.size() - 1; ++n )
        {
            SFVEC2F start3DU( m_seg_points[n].x * aBiuTo3Dunits,
                              -m_seg_points[n].y * aBiuTo3Dunits );

            SFVEC2F end3DU( m_seg_points[n + 1].x * aBiuTo3Dunits,
                            -m_seg_points[n + 1].y * aBiuTo3Dunits );

            if( Is_segment_a_circle( start3DU, end3DU ) )
                aContainer->Add( new CFILLEDCIRCLE2D( start3DU,
                                                      m_trackseg_rad * aBiuTo3Dunits,
                                                      *this ) );
            else
                aContainer->Add( new CROUNDSEGMENT2D( start3DU,
                                                      end3DU,
                                                      m_Width * aBiuTo3Dunits,
                                                      *this ) );
        }
    }
}
    ...
    if( m_on && m_trackseg && m_trackseg_second && m_set_ok )
    {
        ...
        )
        {
            for( int n = 0; n < m_num_arc_segs; ++n )
            {
                ...
                GRLine( clip_box,
                        aDC,
                        m_seg_outer_points[m_num_arc_outer_points - n - 1] + aOffset,
                        m_seg_outer_points[m_num_arc_outer_points - n - 2] + aOffset,
                        2,
                        aColor );
            }
        }
            ...
            if( m_draw_with_segments )
            {
                for( int n = 0; n < m_num_arc_segs; ++n )
                    GRFillCSegm( clip_box,
                                 aDC,
                                 m_seg_points[n].x + aOffset.x,
                                 m_seg_points[n].y + aOffset.y,
                                 m_seg_points[n + 1].x + aOffset.x,
                                 m_seg_points[n + 1].y + aOffset.y,
                                 m_Width,
                                 aColor );
            }
            else
        ...
        if( showClearance )
        {
            for( int n = 0; n < m_num_arc_segs; ++n )
            {
                ...
                GRLine( clip_box,
                        aDC, 
                        m_seg_clearance_points[m_num_arc_outer_points - n - 1] + aOffset,
                        m_seg_clearance_points[m_num_arc_outer_points - n - 2] + aOffset,
                        2,
                        aColor );
            }
            ...
            GRArc1( clip_box,
                    aDC,
                    m_seg_clearance_points[arc1_first] + aOffset,
                    m_seg_clearance_points[arc1_second] + aOffset,
                    m_pos_start + aOffset,
                    0,
                    aColor );
            GRArc1( clip_box,
                    aDC,
                    m_seg_clearance_points[arc2_first] + aOffset,
                    m_seg_clearance_points[arc2_second] + aOffset,
                    m_pos_end + aOffset,
                    0,
                    aColor );
        }
...
ROUNDEDTRACKSCORNER_ROUTE_EDIT::ROUNDEDTRACKSCORNER_ROUTE_EDIT( const BOARD_ITEM* aParent,
                                                                const TRACK* aTrackSeg,
                                                                const TRACK* aTrackSegSecond,
                                                                const PARAMS aParams,
                                                                const bool aCheckNullTrack
...
bool ROUNDEDTRACKSCORNER_ROUTE_EDIT::ChangeTracks( const TRACK* aNewTrack,
                                                  const TRACK* aNewTrackSecond
                                               )
{
    ReleaseTrackSegs();
    if( aNewTrack && 
        aNewTrackSecond &&
        ( aNewTrack->Type() == PCB_TRACE_T ) &&
        ( aNewTrackSecond->Type() == PCB_TRACE_T )
  )
    {
        m_connected_pos = aNewTrack->GetStart();
        if( SetTrackSeg( aNewTrack, true ) )
        {
            if( m_connected_pos == aNewTrackSecond->GetStart() ||
                m_connected_pos == aNewTrackSecond->GetEnd()
          )
            {
...
void ROUNDEDTRACKSCORNER_ROUTE_EDIT::DrawItem( EDA_DRAW_PANEL* aPanel,
                                              wxDC* aDC,
                                              const COLOR4D aColor,
                                              const wxPoint& aOffset,
                                              const DISPLAY_OPTIONS* aDisplOpts
