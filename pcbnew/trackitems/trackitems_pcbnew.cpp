...
TRACK* TRACKITEMS::DragKeepSlopeSegmentTypeCheck( TRACK* aTrackToPoint,
                                                ...
                                                )
{
    ...
    if( cur_track && aTrackSeg && aTrace )
    {
        if( dynamic_cast<TrackNodeItem::TRACKNODEITEM*>( cur_track ) )
        {
            ...
            TRACK* track_at_point = const_cast<TRACK*>( aTrackSeg )->GetTrack( const_cast<TRACK*>( aTrace ),
                                                                               nullptr,
                                                                               aEndpoint,
                                                                               true,
                                                                               false );
            if( track_at_point ) 
            {
                if( dynamic_cast<TrackNodeItem::TRACKNODEITEM*>( track_at_point ) )
                {
                    ...
                    TRACK* last_check_track_at_point = const_cast<TRACK*>( aTrackSeg )->GetTrack( const_cast<TRACK*>( aTrace ),
                                                                                                  nullptr,
                                                                                                  aEndpoint,
                                                                                                  true,
                                                                                                  false );
...
bool TRACKITEMS::DragKeepSlopeSegmentsNumCheck( const bool aCurrentError,
                                                TRACK* aTrackToPoint,
                                                const TRACK *aTrackSeg,
                                                const TRACK *aTrace,
                                                const ENDPOINT_T aEndpoint
                                              )
{
    ...
    if( aTrackToPoint && aTrackSeg && aTrace )
    {
        ...
        {
            TRACK* track = const_cast<TRACK*>( aTrackSeg );
            aTrackToPoint->SetState( BUSY, true );
            TRACK* track_at_point = track->GetTrack( const_cast<TRACK*>( aTrace ),
                                                     nullptr,
                                                     aEndpoint,
                                                     true,
                                                     false );
            if( track_at_point )
            {
                ...
                {
                    track_at_point->SetState( BUSY, true );
                    TRACK* another_track_at_point = track->GetTrack( const_cast<TRACK*>( aTrace ),
                                                                     nullptr,
                                                                     aEndpoint,
                                                                     true,
                                                                     false );
                    if( another_track_at_point ) 
                    {
                        if( dynamic_cast<TrackNodeItem::TRACKNODEITEM*>( another_track_at_point ) )
                        {
                            ...
                            TRACK* last_check_track_at_point = track->GetTrack( const_cast<TRACK*>( aTrace ),
                                                                                nullptr,
                                                                                aEndpoint,
                                                                                true,
                                                                                false );
...
{
    std::unique_ptr<TRACKS_PROGRESS_MARK_SHARP_ANGLES> sharp_marks(
        new TRACKS_PROGRESS_MARK_SHARP_ANGLES( this, aTracksAt, aDRC ) );
    if( sharp_marks )
        sharp_marks->Execute();
}

TRACKITEMS::TRACKS_PROGRESS_MARK_SHARP_ANGLES::TRACKS_PROGRESS_MARK_SHARP_ANGLES( const TRACKITEMS* aParent,
                                                                                  const DLIST<TRACK>* aTracks,
                                                                                  DRC* aDRC
                                                                                ) :
    TRACKITEMS_TRACKS_PROGRESS( aParent, aTracks, nullptr )
{
    ...
    if( track_seg && ( track_seg->Type() == PCB_TRACE_T ) )
    {
        for( int n = 0; n < 2; ++n )
        {
            ...
            if( !via && !pad )
            {
                ...
                BOARD_CONNECTED_ITEM* in_item = m_Parent->m_Board->GetLockPoint( track_pos,
                ...
                if( !in_item || item_not_pad )
                {
                    ...
                    if( t_tracks )
                    {
                        if( m_Parent->Teardrops()->Get( track_seg, track_pos ) ||
                            ( m_Parent->Teardrops()->Get( track_next, track_pos ) &&
                            m_Parent->Teardrops()->Get( track_back, track_pos ) ) )
                            break;
                    }
                    ...
                    if( tracks_list.size() > 2 )
                    {
                        ...
                        {
                            TRACKNODEITEM* item = m_Parent->Teardrops()->Get( trck_sg, track_pos );
                    ...
                    if( !already_marked )
                    {
                        ...
                        if( sharp_track )
                        {
                            m_Parent->DRC_AddMarker( track_seg,
                                                     sharp_track,
                                                     track_pos,
                                                     DRCE_TRACKNODEITEM_UNSPECIFIED );
                            ++num_marks;
...
TRACKITEMS::NET_SCAN_SHARP_ANGLES::NET_SCAN_SHARP_ANGLES( const TRACK* aTrackSeg,
...
TRACK* TRACKITEMS::TRACKS_PROGRESS_MARK_SHARP_ANGLES::Find_Tracks_Sharp_Angle( const TRACK* aTrackSegAt,
                                                                             ...
                                                                             ) const
{
    ...
    {
        std::unique_ptr<NET_SCAN_SHARP_ANGLES> track(
            new NET_SCAN_SHARP_ANGLES( aTrackSegAt, m_Parent, aPosAt ) );
        if( track )
        {
            track->Execute();
    ...
    if( track_seg && ( track_seg->Type() == PCB_TRACE_T ) )
    {
        ...
        for( int n = 0; n < 2; ++n )
        {
            ...
            if( !via && !pad ) //If not connected.
            {
                ...
                TEARDROP* junc = dynamic_cast<TEARDROP*>( m_Parent->Teardrops()->Get( track_seg,
                                                                                      track_pos ) );
                ...
                if( in_item )
                {
                    if( in_item->Type() == PCB_PAD_T )
                    {
                        ...
                        if( junc )
                        {
                            ...
                            m_Parent->Teardrops()->Add( track_seg,
                                                        in_item,
                                                        m_undoredo_items,
                                                        item_pos );
                        }
                ...
                VIA* via_at = m_Parent->GetBadConnectedVia( track_seg,
                                                            track_pos,
                                                            &result_list );
                if( via_at )
                {
...
void TRACKITEMS::Popup_PickTrackOrViaWidth( wxMenu* aMenu, const TRACK* aTrackSeg ) const
{
    if( aMenu && aTrackSeg )
    {
        ...
        if( aTrackSeg->Type() == PCB_VIA_T )
        {
            ...
            if( ( via_size != m_Board->GetDesignSettings().GetCurrentViaSize() ) ||
                ( via_drill != m_Board->GetDesignSettings().GetCurrentViaDrill() ) )
            {
                wxString txt_width = StringFromValue( g_UserUnit, via_size, true );
                wxString txt_drill = StringFromValue( g_UserUnit, via_drill, true );

                msg.Printf( _( "Copy Via Settings [%s / %s] to Current" ),
                            GetChars( txt_width ),
                            GetChars( txt_drill ) );
        ...
        else
        {
            ...
            if( track_width != m_Board->GetDesignSettings().GetCurrentTrackWidth() )
            {
                ...
                msg.Printf( _( "Copy Track Width [%s] to Current" ),
                            GetChars( txt_width ) );
